        ...
        window.fetch = (function () {
            ...
            return async function (...args) {
                ...
                if (nominatim) {
                    ...
                    if (window._customCoordinates) {
                        let c = window._customCoordinates;

                        let customResponce = await window.sgs.customReverse({ lat: parseFloat(lat), lng: parseFloat(lng) }, c.coordinatesObject);

                        c.keys.forEach(k => {
                            if (typeof k !== "string"){
                                alert('Insta-Streaks says: All keys have to be strings');
                                return;
                            }
                            response.address[k] = customResponce.error
                                ? c.customError || response?.address?.country || "Nope"
                    ...
                    if (window._customCoordinates) {
                        ...
                        c.keys.forEach(k => {
                            ...
                            response[k] = customResponce.error
                                ? c.customError || response?.countryCode || "Nope"
                                : customResponce.response;
                        });
                    }
